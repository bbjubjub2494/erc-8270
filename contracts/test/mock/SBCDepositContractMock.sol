...
// forge-lint: disable-next-item(erc20-unchecked-transfer, unsafe-typecast,mixed-case-function,mixed-case-variable,screaming-snake-case-immutable)
/// @notice This is copied from the Gnosis deposit contract and only slightly adapted.
contract SBCDepositContractMock is ISBCDepositContract, IERC165 {
    ...
    function onTokenTransfer(address, uint256 stake_amount, bytes calldata data) external override returns (bool) {
    ...
    function supportsInterface(bytes4 interfaceId) external pure override returns (bool) {
        return interfaceId == type(IERC165).interfaceId || interfaceId == type(ISBCDepositContract).interfaceId
    ...
    /// @notice Compute deposit data root (`DepositData` hash tree root)
    ...
    function _computeDepositDataRoot(
        ...
        return sha256(
            abi.encodePacked(
                ...
                sha256(abi.encodePacked(to_little_endian_64(uint64(32 * amount / 1e9)), bytes24(0), signatureRoot))
            )
        );
    }
    ...
    function claimWithdrawal(address _address) public {
        ...
        if (amount > 0) {
            ...
            stake_token.transfer(_address, amount);
        }
    }
    ...
    {
        require(
            msg.sender == SYSTEM_WITHDRAWAL_EXECUTOR,
